...
async def prepare_transcript(bot: ModMail, channel_id: int, guild_id: int, guild_data: Optional[dict] = None):
    ...
    if guild_transcripts_channel is not None:
        await guild_transcripts_channel.send(content=f"You can view this ticket here: https://mail-hook.xyz/viewticket/{guild_id}/{randomly_generator_id}", file=discord.File(BytesIO(text.encode("utf-8")), filename=f"{channel.name}.txt"))
