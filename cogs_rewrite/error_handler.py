...
class ErrorHandling(commands.Cog):
    ...
    async def on_command_error(self, ctx: Union[commands.Context, InteractionContext], error):
        if isinstance(error, commands.CommandOnCooldown):
            ...
            await ctx.reply(f"Hey there, looks like {error.user} is already in a different modmail thread from another server.\nPlease contact them via DMs or some other method.")
            ...
            await ctx.reply(f"Sorry, you cannot use this command on bots.\nPlease try it on a human being.")
        else:
